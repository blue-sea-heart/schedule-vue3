...
public class ScheduleRepositoryWithBagRelationshipsImpl implements ScheduleRepositoryWithBagRelationships {
    ...
    public Optional<Schedule> fetchBagRelationships(Optional<Schedule> schedule) {
        return schedule.map(this::fetchTags);
    }
    ...
    public List<Schedule> fetchBagRelationships(List<Schedule> schedules) {
        return Optional.of(schedules).map(this::fetchTags).orElse(Collections.emptyList());
    }

    Schedule fetchTags(Schedule result) {
        return entityManager
            .createQuery("select schedule from Schedule schedule left join fetch schedule.tags where schedule.id = :id", Schedule.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Schedule> fetchTags(List<Schedule> schedules) {
        ...
        List<Schedule> result = entityManager
            .createQuery(
                "select schedule from Schedule schedule left join fetch schedule.tags where schedule in :schedules",
                Schedule.class
            )
