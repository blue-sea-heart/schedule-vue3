...
import static com.blueseaheart.demo.domain.TagAsserts.*;
...
import com.blueseaheart.demo.repository.TagRepository;
import com.blueseaheart.demo.service.dto.TagDTO;
import com.blueseaheart.demo.service.mapper.TagMapper;
...
class TagResourceIT {
    ...
    public static Tag createEntity() {
        return new Tag().name(DEFAULT_NAME).color(DEFAULT_COLOR);
    }
    ...
    public static Tag createUpdatedEntity() {
        return new Tag().name(UPDATED_NAME).color(UPDATED_COLOR);
    }
    ...
    @Test
    @Transactional
    void getAllTagsBySchedulesIsEqualToSomething() throws Exception {
        Schedule schedules;
        if (TestUtil.findAll(em, Schedule.class).isEmpty()) {
            tagRepository.saveAndFlush(tag);
            schedules = ScheduleResourceIT.createEntity();
        } else {
            schedules = TestUtil.findAll(em, Schedule.class).get(0);
        }
        em.persist(schedules);
        em.flush();
        tag.addSchedules(schedules);
        tagRepository.saveAndFlush(tag);
        Long schedulesId = schedules.getId();
        // Get all the tagList where schedules equals to schedulesId
        defaultTagShouldBeFound("schedulesId.equals=" + schedulesId);

        // Get all the tagList where schedules equals to (schedulesId + 1)
        defaultTagShouldNotBeFound("schedulesId.equals=" + (schedulesId + 1));
    }
